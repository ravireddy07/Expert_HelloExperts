...
class NumberClass {
	private:
		vector<T> vect;
	public:
		NumberClass(vector<T> &TempVect, T size) { //getting the values to get initilized for vector
			for(T i=0;i<size;i++) {
...
int main() {
	...
	vector<int>myNum(arr, arr+n);
	NumberClass <int> myNumObject(myNum, n);
	myNumObject.printNumber(n); //Printing
	//myNumObject.printNumberWithDescription(n); //Printing
